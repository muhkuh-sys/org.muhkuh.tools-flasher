...
static NETX_CONSOLEAPP_RESULT_T opMode_smart_erase(tFlasherInputParameter *ptAppParams)
{
	...
	case BUS_SPI:
		/*  use SPI flash */
		uprintf("DEBUG: ulEndAdr from Main: %0x08x\n", ptAppParams->uParameter.tSmart_Erase.ulEndAdr);
		...
		// DEBUG INFO: funktioniert nicht
		tResult = spi_smart_erase(&(ptAppParams->uParameter.tSmart_Erase));
		break;
	}
	return tResult;
}
...
static NETX_CONSOLEAPP_RESULT_T check_params(NETX_CONSOLEAPP_PARAMETER_T *ptConsoleParams)
{
	...
	if (ulPars & FLAG_DEVICE)
	{
		...
		else
		{
			...
			if ((ulPars & FLAG_ENDADR) && ulEndAdr > ulFlashSize && (tOpMode != OPERATION_MODE_GetEraseArea && ulEndAdr != 0xffffffffU))
			{
