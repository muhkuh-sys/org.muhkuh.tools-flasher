...
OPERATION_MODE_FlashRandom       = ${OPERATION_MODE_FlashRandom}
...
-- Writes pseudorandom data ulStartAddr in the flash.
-- The data is generated by the flasher. ulDataAddress is used as the buffer address.
function flash_random(tPlugin, aAttr, ulStartAdr, ulDataByteSize, ulDataAddress, fnCallbackMessage, fnCallbackProgress)
	local aulParameter = 
	{
		OPERATION_MODE_FlashRandom,
		aAttr.ulDeviceDesc,
		ulStartAdr,
		ulDataByteSize,
		ulDataAddress
...
function flashAreaRandom(tPlugin, aAttr, ulDeviceOffset, ulDataByteSize, fnCallbackMessage, fnCallbackProgress)
	local fOk
	local ulBufferAdr = aAttr.ulBufferAdr

	print(string.format("flashing random data at offset 0x%08x-0x%08x.", ulDeviceOffset, ulDeviceOffset+ulDataByteSize))
	fOk = flasher.flash_random(tPlugin, aAttr, ulDeviceOffset, ulDataByteSize, ulBufferAdr, fnCallbackMessage, fnCallbackProgress)
	if fOk then
		return true, "Random test succeeded."
	else
		return false, "Random test failed!"
	end
end
