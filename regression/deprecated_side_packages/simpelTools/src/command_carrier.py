...
class command_carrier:
    def __init__(self, ):
        ...
        self.ID = None  # a ID used for the command to be executed.
        self.time_start = None
        self.time_end = None
        self.time_execute = None
        self.cmd = ''  # command to be executed
        self.executed = False
        self.change_dir_to_bin = False

        self.ret_val = None  # returnvalue of executed program
        self.str_stdout = ''  # if used, will return the output string of the program
        self.str_stderr = ''  # if used, will return the output string of the program

        # (prio 1) dirct log to upper consol
        self.direct_out = False  # activates direct stdout/err for seeing the progress, but not capturing it.
        self.direct_err = False

        # (prio 2) path for output files
        self.path_stdout = None  # path for storing logmessages from stdout (if set will not print them)
        self.path_stderr = None  # path for storing logmessages from stderr (if set will not print them)
        ...
        # used in function
        self.stream_stdout = None  # stream to used for communication. may be default stdx or a file to write to.
        self.stream_stderr = None  # stream to used for communication. may be default stdx or a file to write to.
...
def helper_gen_footer(cmd_carry, form):
    tmp = "---END LOG---\n\n%s\n[pirate][file][%s][cmd]: %s\nReal runtime: %fs\nReturn value: %d" % (
        datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'), form, cmd_carry.cmd, cmd_carry.time_execute,
        cmd_carry.ret_val)
    tmp_str = str(tmp)
    return tmp_str


def runCommandBase(cmd_carry, user_input=None):
    '''
    ...
    if user_input:
        b_stdout, b_stderr = p.communicate(input=user_input)
    else:
...
def batch_command_base(cmd_carry_default, array_command, path_logfile='.',
    ...
    result = []
    l.info(
        "Representation of executed commands are reduced by the absoulte path. Only filename and binary are displayed!")
    for idx, cmd in enumerate(array_command):
        ...
        if idx in user_input_assign.keys():
            runCommandBase(tmp_batch_cpy, user_input_assign[idx])
        else:
